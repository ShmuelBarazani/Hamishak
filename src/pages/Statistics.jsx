...
// 🗓️ שלבי הנוק-אאוט מעבר ל-1/16 — לפי הבראקט הרשמי של FIFA. תאריכים/שעות בשעון ישראל.
...
const WC_KO_ROUNDS = [
  ...
  // 🥉 קרב על המקום השלישי — מפסידות חצאי הגמר. 17:00 ET בשבת = 00:00 שעון ישראל,
  //    כלומר יום ראשון 19.7 — אותו יום קלנדרי כמו הגמר (22:00).
  { third:true, lvl:4, idx:0, mon:7, day:19, time:'00:00', stage:'קרב על המקום השלישי 🥉' },
  { lvl:4, idx:0, mon:7, day:19, time:'22:00', stage:'הגמר 🏆' },
];

// הבלוק בעץ הבראקט (טווח משחקי 1/16) עבור צד מסוים של משחק נוק-אאוט
const koSlotBlock = (lvl, idx, side) => {
...
const koAliveInBlock = (start, len, el) => {
  const out=[];
  for(let i=start;i<start+len && i<WC_BRACKET_ORDER.length;i++)
    WC_BRACKET_ORDER[i].forEach(t=>{ if(!el.has(normalizeTeam(t))) out.push(t); });
  return out;
};
...
const loadAllPreds = async gameId => {
  ...
  try {
    ...
    for (let from = 0; from < count; from += CHUNK) {
      jobs.push(
        supabase.from(src).select(PRED_COLS).eq('game_id', gameId)
...
function computeInsights(allQuestions, allPredictions, teams, myPredByQid = {}, scoreByName = {}) {
  ...
  // ── 🆕 11. שאלות הרצח 💀 ─────────────────────────────────────────────
  {
    ...
    allQuestions.filter(q=>q.table_id!=='T1'&&q.actual_result?.trim()&&q.actual_result!=='__CLEAR__').forEach(q=>{
      ...
      // תוצאה עשויה להכיל כמה תשובות תקפות מופרדות ב-||| (תיקו/שוויון) — ניחוש שתואם לאחת מהן נחשב נכון
      const actualSet=new Set(q.actual_result.split('|||').map(a=>normPred(a.trim())).filter(Boolean));
      const heroes=qPreds.filter(p=>actualSet.has(normPred(p.text_prediction.trim()))).map(p=>p.participant_name);
    ...
    if(top.length>0){
      insights.push({
        ...
        murderData:top.map(h=>({
          ...
          actual:h.q.actual_result.split('|||').map(a=>a.trim()).join(' / '), correct:h.correct, total:h.total, pctVal:pct(h.correct,h.total),
  ...
  // ── 🎯 ניחושי העולות — סט A (פגיעות בפועל) + סט B (פוטנציאל לפי שיבוץ) ──────
  {
    ...
    // התפלגות פגיעות: counts=[{name,hit}] → {distData, summary}
    const buildDist = (counts, N, unit) => {
      ...
      const maxHit=Math.max(...counts.map(x=>x.hit));
      const peak=counts.filter(x=>x.hit===maxHit).length;
      const avg=counts.reduce((s,x)=>s+x.hit,0)/counts.length;
      return { distData:buckets, summary:`${peak} ${peak===1?'משתתף':'משתתפים'} בשיא (${maxHit} מתוך ${N} ${unit}) • ממוצע ${avg.toFixed(1)}` };
    };

    // ===== ניחושי 1/16 לכל משתתף (T16 ראש/סגנית + T17 שלישיות שהעפילו) =====
    const t16 = allQuestions.filter(q=>q.table_id==='T16' && !String(q.question_id).includes('.'));
    const actualR32 = new Set();
    t16.forEach(q=>{ if(isClosed(q.actual_result)) q.actual_result.split('|||').forEach(t=>{const n=normT(t);if(n)actualR32.add(n);}); });
    const t17main={}, t17adv={};
    allQuestions.filter(q=>q.table_id==='T17').forEach(q=>{const qid=String(q.question_id);if(qid.includes('.'))t17adv[qid.split('.')[0]]=q;else t17main[qid]=q;});
    Object.keys(t17main).forEach(g=>{const third=t17main[g].actual_result,adv=t17adv[g]?.actual_result;if(isClosed(third)&&(adv||'').trim()==='כן')third.split('|||').forEach(t=>{const n=normT(t);if(n)actualR32.add(n);});});
    const predR32={};
    {
      ...
      const partThird={},partAdv={};
      preds.forEach(p=>{const v=p.text_prediction?.trim();if(!v)return;
      ...
      Object.keys(partThird).forEach(name=>{Object.keys(partThird[name]).forEach(g=>{if(partAdv[name]?.[g]==='כן')(predR32[name]??=new Set()).add(partThird[name][g]);});});
    }

    // ניחושי/תוצאות טבלאות "נבחרת עולה"
    const predOf = tbl => { const ids=new Set(allQuestions.filter(q=>q.table_id===tbl).map(q=>q.id)); const m={}; preds.forEach(p=>{const v=p.text_prediction?.trim();if(v&&ids.has(p.question_id))(m[p.participant_name]??=new Set()).add(normT(v));}); return m; };
    const actualOf = tbl => { const s=new Set(); allQuestions.filter(q=>q.table_id===tbl).forEach(q=>{if(isClosed(q.actual_result))q.actual_result.split('|||').forEach(t=>{const n=normT(t);if(n)s.add(n);});}); return s; };

    // ===== סט A: פגיעות בפועל — כרטיס אחד עם טאבים =====
    const hitsDist = (predBy, actualSet, N, unit) => {
      const names=Object.keys(predBy);
      if(names.length===0||actualSet.size===0) return null;
      const counts=names.map(name=>{let hit=0;(predBy[name]||new Set()).forEach(t=>{if(actualSet.has(t))hit++;});return {name,hit};});
      return buildDist(counts, N, unit);
    };
    const stageDefs=[
      {key:'r32',label:'שלב 1/16',pred:predR32,actual:actualR32},
      {key:'t19',label:'שמינית',pred:predOf('T19'),actual:actualOf('T19')},
      {key:'t21',label:'רבע',pred:predOf('T21'),actual:actualOf('T21')},
      {key:'t23',label:'חצי',pred:predOf('T23'),actual:actualOf('T23')},
      {key:'t25',label:'גמר',pred:predOf('T25'),actual:actualOf('T25')},
    ];
    const stagesA=stageDefs.map(s=>{
      const d=hitsDist(s.pred,s.actual,s.actual.size,'עולות');
...
function InsightCard({ insight, me }) {
  ...
  const [stageTab, setStageTab] = useState(insight.defaultStage||0);
  ...
  const renderChart = () => {
    ...
    // 🎯/🔮 advmulti — כרטיס אחד עם טאבי שלבים + היסטוגרמה לחיצה
    if (insight.chartType === 'advmulti') {
      const stages = insight.stages || [];
      const cur = stages[stageTab] || stages[insight.defaultStage||0] || stages[0] || {};
      const data = cur.distData || [];
      ...
      return (
        <div style={{marginTop:2}}>
          ...
          <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:8}}>
            {stages.map((s,i)=>(
              <button key={s.key+i} onClick={()=>{ if(s.available){setStageTab(i);setProfOpen(null);} }}
                style={{padding:'4px 11px',borderRadius:7,fontSize:'0.74rem',fontWeight:700,cursor:s.available?'pointer':'not-allowed',
                  border:`1px solid ${i===stageTab?insight.color:'rgba(148,163,184,0.3)'}`,
                  background:i===stageTab?insight.color:'transparent',
                  color:i===stageTab?'#0a0f1a':(s.available?'#cbd5e1':'#475569'),opacity:s.available?1:0.55}}>
                {s.label}{!s.available?' 🔒':''}
              </button>
            ))}
          </div>
          ...
          {cur.available?(
            <>
              ...
              {open?(
                ...
                <p style={{color:'#475569',fontSize:'0.68rem',marginTop:6,textAlign:'center'}}>לחץ על עמודה לרשימת המשתתפים</p>
              )}
            </>
          ):(
            <div style={{textAlign:'center',padding:'26px 8px',color:'#64748b',fontSize:'0.8rem'}}>🔒 {cur.emptyMsg}</div>
          )}
        </div>
      );
    }
...
export default function Statistics() {
  ...
  const matchesByDay = useMemo(()=>{
    ...
    if(isWC){
      ...
      // שלב 1/16 — קבוצות ידועות
      WC_KO_FIXTURES.forEach((f,i)=>{
        const key=`${f.mon}-${f.day}`;
        if(!map[key]) map[key]=[];
        map[key].push({
          q:{ id:`ko_${i}`, home_team:f.home, away_team:f.away, homeTeams:[f.home], awayTeams:[f.away], stage_name:f.stage, actual_result:'', table_id:'KO', isKnockout:true },
        ...
        <CardContent>
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {matches.map(({q,time})=>{
              ...
              const hTeams=q.homeTeams||(q.home_team?[q.home_team]:[]);
                ...
                <div key={q.id} style={{position:'relative',borderRadius:10,border:'1px solid rgba(6,182,212,0.15)',background:'rgba(0,0,0,0.25)',padding:'10px 12px'}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,width:'100%'}}>
                    ...
                    <span style={{textAlign:'center',fontWeight:700,color:(q.isKnockout?(koSc?'#fde68a':'#64748b'):(st.hasActual?'#fde68a':'#64748b')),fontSize:'0.95rem',width:56,flexShrink:0}}>
                      {q.isKnockout?(koResolved?(koSc?`${koSc[1]} - ${koSc[2]}`:'? - ?'):'🆚'):(st.hasActual?formatResult(q.actual_result):'? - ?')}
                    </span>
