...
export default function Login() {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'register' && !fullName.trim()) {
      toast({ title: 'שגיאה', description: 'נא למלא שם משתמש', variant: 'destructive' });
      return;
    }
    setLoading(true);
          ...
          {mode === 'register' && (
            <div>
              ...
              <div className="relative">
                <User className="absolute right-3 top-2.5 w-4 h-4" style={{ color: '#64748b' }} />
                <Input
                  type="text"
                  value={fullName}
                  onChange={e => setFullName(e.target.value)}
                  placeholder="השם שיופיע בטבלת הדירוג"
                  className="pr-9"
              ...
              <p className="text-xs mt-1" style={{ color: '#475569' }}>
                השם שיופיע בלוח הדירוג ובניחושים
              </p>
            </div>
          )}
