...
export default function UserManagement() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      participants.forEach(p => {
        if (!gamesMap[p.user_email]) gamesMap[p.user_email] = [];
        const game = games.find(g => g.id === p.game_id);
        if (game) gamesMap[p.user_email].push({ ...game, participant: p });
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      const predictions = await db.Prediction.filter({ participant_name: userToDelete.full_name }, null, 10000);
      for (const pred of predictions) await db.Prediction.delete(pred.id);
  ...
  const handleToggleAdmin = async (user) => {
    if (user.email === currentUser?.email) { toast({ title: "לא ניתן", description: "אינך יכול לשנות את ההרשאות של עצמך", variant: "destructive" }); return; }
    try {
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <Alert variant="destructive">
          ...
          <AlertDescription>אין לך הרשאות לגשת לדף זה. רק מנהלים יכולים לנהל משתמשים.</AlertDescription>
        </Alert>
      </div>
    );
  }
  ...
  const appUrl = window.location.origin + window.location.pathname;
  const roleText = newUser.role === 'admin' ? 'מנהל' : newUser.role === 'predictor' ? 'מנחש' : 'צופה';
  const platformAdminParticipants = users.filter(u => ADMIN_EMAILS.includes(u.email));
  const gameParticipants = users.filter(u => !ADMIN_EMAILS.includes(u.email));
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 flex justify-between items-start">
          <div>
            ...
            <p style={{ color: '#94a3b8' }}>סה"כ {users.length} משתמשים | {allGames.length} משחקים</p>
          </div>
        ...
        <Card className="mb-6" style={{ background: 'rgba(139, 92, 246, 0.08)', border: '1px solid rgba(139, 92, 246, 0.4)' }}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2" style={{ color: '#a78bfa' }}>
              ...
              מנהלי פלטפורמה (Supabase Auth)
              <Badge style={{ background: 'rgba(139, 92, 246, 0.3)', color: '#a78bfa', border: '1px solid rgba(139, 92, 246, 0.4)' }}>{ADMIN_EMAILS.length}</Badge>
            </CardTitle>
            <p className="text-sm" style={{ color: '#94a3b8' }}>מוגדרים ברמת Supabase Auth — גישה לכל המשחקים ודפי ניהול, ללא צורך ב-GameParticipant</p>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {ADMIN_EMAILS.map((email) => {
                const isCurrent = email === currentUser?.email;
                const hasParticipant = platformAdminParticipants.find(u => u.email === email);
                return (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          <Badge style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#10b981', border: '1px solid rgba(16, 185, 129, 0.3)', fontSize: '10px' }}>
                            app_metadata ✓
                          </Badge>
                          {hasParticipant && (
            ...
            <Alert className="mt-4" style={{ background: 'rgba(139, 92, 246, 0.08)', border: '1px solid rgba(139, 92, 246, 0.25)' }}>
              <Info className="w-4 h-4" style={{ color: '#a78bfa' }} />
              <AlertDescription style={{ color: '#94a3b8', fontSize: '13px' }}>
                <strong style={{ color: '#a78bfa' }}>ההבדל בין סוגי מנהלים:</strong><br />
                • <strong style={{ color: '#a78bfa' }}>מנהל פלטפורמה</strong> — מוגדר ב-Supabase Auth, רואה הכל, לא צריך להיות GameParticipant<br />
                • <strong style={{ color: '#06b6d4' }}>מנהל משחק</strong> — GameParticipant עם role=admin, מנהל משחק ספציפי<br />
                • ניתן להיות שניהם בו-זמנית ✓
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
        ...
        <Card style={{ background: 'rgba(30, 41, 59, 0.6)', border: '1px solid rgba(6, 182, 212, 0.2)' }}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2" style={{ color: '#06b6d4' }}>
              ...
              משתמשי המשחק (GameParticipants)
              <Badge style={{ background: 'rgba(6, 182, 212, 0.2)', color: '#06b6d4', border: '1px solid rgba(6, 182, 212, 0.3)' }}>{gameParticipants.length}</Badge>
            </CardTitle>
            <p className="text-sm" style={{ color: '#94a3b8' }}>משתמשים הרשומים למשחקים כמנחשים, צופים או מנהלי משחק</p>
          </CardHeader>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {gameParticipants.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="p-8 text-center" style={{ color: '#64748b' }}>אין משתמשים רשומים עדיין</td>
                    </tr>
                      ...
                      <tr key={user.id} className="hover:bg-cyan-500/10 transition-colors" style={{ borderBottom: '1px solid rgba(6, 182, 212, 0.1)' }}>
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span style={{ color: '#f8fafc', fontWeight: '500' }}>{user.full_name}</span>
                        ...
                        <td className="text-center p-4">
                          {user.role === 'admin' ? (
                            ...
                            <Badge style={{ background: 'rgba(148, 163, 184, 0.2)', color: '#94a3b8', border: '1px solid rgba(148, 163, 184, 0.3)' }}>צופה</Badge>
                          )}
                        </td>
                        <td className="p-4">
                          {games.length === 0 ? (
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-2">
                            <Button onClick={() => handleToggleAdmin(user)} disabled={isCurrentUser} size="sm" variant="outline"
                              style={{ borderColor: user.role === 'admin' ? '#ef4444' : '#06b6d4', color: user.role === 'admin' ? '#fca5a5' : '#06b6d4', background: 'rgba(30, 41, 59, 0.4)' }}
        ...
        <Dialog open={showAddUserDialog} onOpenChange={setShowAddUserDialog}>
          <DialogContent style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', border: '1px solid rgba(6, 182, 212, 0.3)' }} dir="rtl">
            <DialogHeader>
              ...
              <DialogDescription style={{ color: '#94a3b8' }}>הזן פרטי משתמש — המערכת תציג פרטי התחברות להעתקה ושליחה</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              {[{ label: 'שם מלא', key: 'full_name', type: 'text', placeholder: 'הזן שם מלא...' }, { label: 'אימייל', key: 'email', type: 'email', placeholder: 'example@email.com' }, { label: 'סיסמה', key: 'password', type: 'text', placeholder: 'לפחות 6 תווים...' }].map(({ label, key, type, placeholder }) => (
                <div key={key}>
                  <label className="text-sm font-medium mb-2 block" style={{ color: '#94a3b8' }}>{label}</label>
                  <Input type={type} value={newUser[key]} onChange={(e) => setNewUser({...newUser, [key]: e.target.value})} placeholder={placeholder} style={{ background: '#0f172a', borderColor: 'rgba(6, 182, 212, 0.3)', color: '#f8fafc' }} />
                </div>
              ))}
              <div>
        ...
        <Dialog open={showCredentialsDialog} onOpenChange={setShowCredentialsDialog}>
          <DialogContent style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', border: '1px solid rgba(6, 182, 212, 0.3)' }} dir="rtl">
            <DialogHeader>
              ...
              <DialogDescription style={{ color: '#94a3b8' }}>העתק ושלח למשתמש ב-WhatsApp</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="flex gap-3 justify-end">
                <Button variant="outline" onClick={() => { setShowCredentialsDialog(false); setNewUser({ full_name: "", email: "", password: "", role: "predictor" }); }} style={{ borderColor: 'rgba(148, 163, 184, 0.3)', color: '#94a3b8', background: 'transparent' }}>סגור</Button>
                <Button onClick={copyCredentialsToClipboard} style={{ background: credentialsCopied ? 'linear-gradient(135deg, #10b981, #059669)' : 'linear-gradient(135deg, #06b6d4, #0ea5e9)', color: 'white' }}>
              ...
              <div className="space-y-4">
                <Alert style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
                  <AlertDescription style={{ color: '#fca5a5' }}>
                    ...
                    <p>{userToDelete.full_name} — {userToDelete.email}</p>
                  </AlertDescription>
                </Alert>
                <div className="flex gap-3 justify-end">
                  ...
                  <Button onClick={confirmDelete} disabled={deletingUser} style={{ background: 'linear-gradient(135deg, #ef4444, #dc2626)', color: 'white' }}>
