...
// ✅ בדיקת role — תומך ב-app_metadata, user_metadata ו-role ישיר
...
export default function UserManagement() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // ✅ תיקון: בדיקת role נכונה
      const userRole = getUserRole(currentUserData);
      if (userRole !== 'admin') {
        toast({
          title: "אין הרשאות",
      ...
      participants.forEach(p => {
        ...
        if (!gamesMap[mapKey]) gamesMap[mapKey] = [];
        const game = games.find(g => g.id === p.game_id);
        if (game) gamesMap[mapKey].push({ ...game, participant: p });
      });
...
בהצלחה! 🏆`;
    ...
    toast({ title: "הועתק ללוח!", description: "כעת תוכל לשלוח את הפרטים ב-WhatsApp או SMS", className: "bg-green-900/30 border-green-500 text-green-200" });
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      for (const pred of predictions) await Prediction.delete(pred.id);

      const allRecords = await db.GameParticipant.filter({ user_email: userToDelete.user_email }, null, 1000);
      for (const rec of allRecords) await db.GameParticipant.delete(rec.id);

      toast({ title: "נמחק בהצלחה!", description: `המשתמש ${userToDelete.participant_name || userToDelete.user_email} וכל הניחושים שלו נמחקו`, className: "bg-green-900/30 border-green-500 text-green-200" });
      await loadData();
    } catch (error) {
  ...
  const handleToggleAdmin = async (user) => {
    ...
    try {
      ...
      toast({ title: "עודכן!", description: `${user.participant_name || user.user_email || '—'} ${newRole === 'admin' ? 'הוגדר כמנהל' : 'הוסר מהרשאות מנהל'}`, className: "bg-cyan-900/30 border-cyan-500 text-cyan-200" });
      await loadData();
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p style={{ color: '#94a3b8' }}>סה"כ {users.length} משתמשים במערכת | {allGames.length} משחקים</p>
            </div>
          ...
          <Alert className="mt-6" style={{ background: 'rgba(6, 182, 212, 0.1)', border: '1px solid rgba(6, 182, 212, 0.3)' }}>
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {users.map((user) => {
                    const games = userGames[user.user_email] || userGames[`__name__:${user.participant_name}`] || [];
                    return (
                      <tr key={user.id} className="hover:bg-cyan-500/10 transition-colors" style={{ borderBottom: '1px solid rgba(6, 182, 212, 0.1)' }}>
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <UserIcon className="w-4 h-4" style={{ color: '#06b6d4' }} />
                            <span style={{ color: '#f8fafc', fontWeight: '500' }}>{user.participant_name || user.user_email || '—'}</span>
                            {user.id === currentUser.id && (
                              <Badge className="text-xs" style={{ background: '#8b5cf6', color: 'white' }}>את/ה</Badge>
                            )}
                          </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="text-center p-4">
                          {user.role_in_game === 'admin' ? (
                            <Badge className="text-white" style={{ background: 'linear-gradient(135deg, #06b6d4 0%, #0ea5e9 100%)', boxShadow: '0 0 10px rgba(6, 182, 212, 0.4)' }}>
                              <Shield className="w-3 h-3 ml-1" />מנהל מערכת
                            </Badge>
                          ) : (
                            <Badge style={{ background: 'rgba(148, 163, 184, 0.2)', color: '#94a3b8', border: '1px solid rgba(148, 163, 184, 0.3)' }}>
                              משתמש רגיל
                            </Badge>
                          )}
                        </td>
                        <td className="p-4">
                          {games.length === 0 ? (
                            ...
                            <div className="flex flex-wrap gap-1">
                              {games.map(g => {
                                const now = new Date();
                                const isActive = g.status === 'active' || (g.start_date && g.end_date && new Date(g.start_date) <= now && new Date(g.end_date) >= now);
                                return (
                                  <Badge key={g.id} variant="outline" style={{ background: isActive ? 'rgba(16, 185, 129, 0.1)' : 'rgba(148, 163, 184, 0.1)', color: isActive ? '#10b981' : '#64748b', border: `1px solid ${isActive ? 'rgba(16, 185, 129, 0.3)' : 'rgba(148, 163, 184, 0.3)'}`, fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: '2px' }}>
                                    {g.participant.role_in_game === 'admin' && <Crown className="w-2.5 h-2.5" />}
                                    {g.game_name}
                                    <span style={{ opacity: 0.7, fontSize: '0.65rem', marginRight: '2px' }}>({isActive ? 'פעיל' : 'סגור'})</span>
                                  </Badge>
                                );
                              })}
                            </div>
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-2">
                            <Button onClick={() => handleToggleAdmin(user)} disabled={user.id === currentUser.id} size="sm" variant="outline"
                            ...
                            <Button onClick={() => handleDeleteUser(user)} disabled={user.id === currentUser.id} size="sm" variant="outline"
        ...
        <Dialog open={showAddUserDialog} onOpenChange={setShowAddUserDialog}>
          <DialogContent style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', border: '1px solid rgba(6, 182, 212, 0.3)', boxShadow: '0 0 40px rgba(6, 182, 212, 0.2)' }} dir="rtl">
            <DialogHeader>
              ...
              <DialogDescription style={{ color: '#94a3b8' }}>הזן פרטי משתמש והמערכת תציג לך את פרטי ההתחברות להעתקה</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium mb-2 block" style={{ color: '#94a3b8' }}>שם מלא</label>
                <Input value={newUser.full_name} onChange={(e) => setNewUser({...newUser, full_name: e.target.value})} placeholder="הזן שם מלא..." style={{ background: '#0f172a', borderColor: 'rgba(6, 182, 212, 0.3)', color: '#f8fafc' }} />
              </div>
              <div>
                <label className="text-sm font-medium mb-2 block" style={{ color: '#94a3b8' }}>אימייל</label>
                <Input type="email" value={newUser.email} onChange={(e) => setNewUser({...newUser, email: e.target.value})} placeholder="example@email.com" style={{ background: '#0f172a', borderColor: 'rgba(6, 182, 212, 0.3)', color: '#f8fafc' }} />
              </div>
              <div>
                <label className="text-sm font-medium mb-2 block" style={{ color: '#94a3b8' }}>סיסמה</label>
                <Input type="text" value={newUser.password} onChange={(e) => setNewUser({...newUser, password: e.target.value})} placeholder="לפחות 6 תווים..." style={{ background: '#0f172a', borderColor: 'rgba(6, 182, 212, 0.3)', color: '#f8fafc' }} />
              </div>
              <div>
              ...
              <div className="flex gap-3 justify-end">
                <Button variant="outline" onClick={() => { setShowAddUserDialog(false); setNewUser({ full_name: "", email: "", password: "", role: "predictor" }); }} style={{ borderColor: 'rgba(148, 163, 184, 0.3)', color: '#94a3b8', background: 'transparent' }}>ביטול</Button>
                <Button onClick={handleAddUser} style={{ background: 'linear-gradient(135deg, #06b6d4 0%, #0ea5e9 100%)', color: 'white', boxShadow: '0 0 20px rgba(6, 182, 212, 0.4)' }}>
        ...
        <Dialog open={showCredentialsDialog} onOpenChange={setShowCredentialsDialog}>
          <DialogContent style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', border: '1px solid rgba(6, 182, 212, 0.3)', boxShadow: '0 0 40px rgba(6, 182, 212, 0.2)' }} dir="rtl">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold flex items-center gap-2" style={{ color: '#06b6d4' }}>
                <UserPlus className="w-6 h-6" />פרטי משתמש חדש - מוכן לשליחה!
              </DialogTitle>
              <DialogDescription style={{ color: '#94a3b8' }}>העתק את הפרטים ושלח למשתמש ב-WhatsApp או SMS</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <Alert style={{ background: 'rgba(251, 191, 36, 0.1)', border: '1px solid rgba(251, 191, 36, 0.3)' }}>
                <AlertDescription style={{ color: '#fdba74' }}>
                  <strong>חשוב:</strong> לחץ על "העתק פרטים" ושלח למשתמש. המשתמש צריך להירשם בעצמו דרך מסך ההתחברות.
                </AlertDescription>
              </Alert>
              <div className="flex gap-3 justify-end">
                <Button variant="outline" onClick={closeCredentialsDialog} style={{ borderColor: 'rgba(148, 163, 184, 0.3)', color: '#94a3b8', background: 'transparent' }}>סגור</Button>
                <Button onClick={copyCredentialsToClipboard} style={{ background: credentialsCopied ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 'linear-gradient(135deg, #06b6d4 0%, #0ea5e9 100%)', color: 'white', boxShadow: '0 0 20px rgba(6, 182, 212, 0.4)' }}>
        ...
        <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <DialogContent style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', border: '1px solid rgba(239, 68, 68, 0.3)', boxShadow: '0 0 40px rgba(239, 68, 68, 0.2)' }} dir="rtl">
            <DialogHeader>
            ...
            {userToDelete && (
              <div className="space-y-4">
                ...
                <div className="flex gap-3 justify-end">
                  <Button variant="outline" onClick={() => { setShowDeleteDialog(false); setUserToDelete(null); }} disabled={deletingUser} style={{ borderColor: 'rgba(148, 163, 184, 0.3)', color: '#94a3b8', background: 'transparent' }}>ביטול</Button>
                  <Button onClick={confirmDelete} disabled={deletingUser} style={{ background: 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', color: 'white' }}>
