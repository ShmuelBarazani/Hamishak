...
export default function LeaderboardNew() {
  ...
  /** שלוף דירוג — מסונן לפי game_id בלבד */
  const loadAllRankings = async (gameId, orderBy = '-current_score') => {
    let all = [], from = 0;
    const PAGE = 1000;
    while (true) {
      let q = supabase.from('rankings').select('*').eq('game_id', gameId).range(from, from + PAGE - 1);
      if (orderBy === '-current_score') q = q.order('current_score', { ascending: false });
      else if (orderBy) q = q.order(orderBy.replace('-', ''), { ascending: !orderBy.startsWith('-') });
  ...
  /**
   * שלוף שאלות — מסונן לפי game_id.
   * לשאלות מיקומים (T14-T17, T19) שמגיעות ממשחק אחר (שלב הבתים)
   * נשלוף בנפרד ונמזג.
   */
  const loadAllQuestions = async (gameId) => {
    ...
    while (true) {
      const { data, error } = await supabase
        .from('questions').select('*').eq('game_id', gameId)
    ...
    // אם שאלות מיקומים לא נמצאו במשחק הנוכחי — שלוף מכל המשחקים
    const hasLocation = all.some(q => LOCATION_IDS.includes(q.table_id));
    if (!hasLocation) {
      const { data: locQ } = await supabase
        .from('questions').select('*').in('table_id', LOCATION_IDS);
      if (locQ && locQ.length > 0) all = [...all, ...locQ];
    }
  ...
  /**
   * שלוף ניחושים של משתתף ספציפי.
   * ─────────────────────────────────────────────────────────────────
   * הפרדה בין משחקים:
   *   • שאלות רגילות  → מסונן לפי game_id (הגנה מפני ערבוב)
   *   • שאלות מיקומים → שאלות ממשחק אחר עם UUID שונה; מסינן לפי
   *                     participant_name בלבד ומסנן אח"כ לפי table_id
   * ─────────────────────────────────────────────────────────────────
   */
  const loadPredictionsForParticipant = async (gameId, participantName, allQuestions) => {
    const LOCATION_IDS = ['T14', 'T15', 'T16', 'T17', 'T19'];

    // קבוצות שאלות
    const gameQuestionIds    = new Set(allQuestions.filter(q => !LOCATION_IDS.includes(q.table_id)).map(q => q.id));
    const locationQuestions  = allQuestions.filter(q => LOCATION_IDS.includes(q.table_id));
    const locationQuestionIds = new Set(locationQuestions.map(q => q.id));
    const allKnownIds        = new Set(allQuestions.map(q => q.id));

    // 1. ניחושים מהמשחק הנוכחי בלבד
    let gamePreds = [], from = 0;
    const PAGE = 1000;
    while (true) {
      const { data, error } = await supabase
        .from('predictions')
        ...
        .range(from, from + PAGE - 1);
      if (error || !data || data.length === 0) break;
      gamePreds = [...gamePreds, ...data];
      if (data.length < PAGE) break;
      from += PAGE;
    }

    // 2. ניחושים לשאלות מיקומים (cross-game lookup לפי question_id UUID)
    //    — רק אם שאלות מיקומים קיימות ולא נמצאו בניחושי המשחק
    const coveredByGame = new Set(gamePreds.map(p => p.question_id));
    const missingLocIds = locationQuestions
      .filter(q => !coveredByGame.has(q.id))
      .map(q => q.id);
    ...
    if (missingLocIds.length > 0) {
      const { data: lp } = await supabase
        ...
        .select('*')
        .eq('participant_name', participantName)
        .in('question_id', missingLocIds);
      if (lp) locPreds = lp;
    }

    // 3. מזג — שמור רק ניחושים לשאלות ידועות
    const merged = [...gamePreds, ...locPreds].filter(p => allKnownIds.has(p.question_id));
   ...
   * בניית predMap חייבת לטפל בשניהם.
   */
  const calcScore = (allQuestions, predictions) => {
    ...
    // ✅ תיקון: home_prediction + away_prediction → "X-Y" (כמו ב-ViewSubmissions)
    const predMap = {};
  ...
  const loadParticipantDetails = async (participantName) => {
    ...
    // פתח dialog עם loading מיד
    setSelectedParticipant(participantName);
    setParticipantDetails(null);
    setLoadingDetails(true);

    try {
      // 1. שאלות — מסונן למשחק הנוכחי + מיקומים cross-game
      const allQuestions = await loadAllQuestions(currentGame.id);

      // 2. ניחושים — עם הפרדת משחקים
      const allPredictions = await loadPredictionsForParticipant(
      ...
      // dedup predictions map לצורך הצגת הניחוש
  ...
  return (
    <div className="min-h-screen p-3 md:p-6" dir="rtl" style={{ background: 'linear-gradient(135deg, var(--bg1) 0%, var(--bg2) 50%, var(--bg1) 100%)' }}>
      <div className="max-w-7xl mx-auto">

        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start gap-3 mb-4 md:mb-8">
          <div>
            <h1 className="text-xl md:text-4xl font-bold mb-1 md:mb-2 flex items-center gap-2 md:gap-3" style={{ color: '#f8fafc', textShadow: '0 0 10px var(--tp-30)' }}>
      ...
      <Dialog open={selectedParticipant !== null} onOpenChange={() => { setSelectedParticipant(null); setParticipantDetails(null); }}>
        <DialogContent
          className="max-w-6xl max-h-[85vh] w-[95vw] md:w-auto flex flex-col"
          style={{
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-4 mt-1 md:mt-2">
              {loadingDetails ? (
                <div className="flex items-center gap-2">
                  <Loader2 className="w-4 h-4 animate-spin" style={{ color: 'var(--tp)' }} />
                  <span className="text-sm" style={{ color: 'var(--tp)' }}>מחשב ניקוד עדכני...</span>
                </div>
              ) : (
                <>
                  ...
                  {participantDetails?.scores?.map((s, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-white/5" style={{ backgroundColor: 'var(--bg3)' }}>
                        ...
                        <td className="text-center p-1.5">
                          <span className="text-xs" style={{ color: '#94a3b8' }}>{s.prediction || '-'}</span>
                        </td>
                        <td className="text-center p-1.5">
                          <span className="text-xs" style={{ color: '#f8fafc' }}>{s.actual_result || '-'}</span>
                        </td>
                        <td className="text-center p-1.5">
