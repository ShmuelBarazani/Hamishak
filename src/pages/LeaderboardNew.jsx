...
export default function LeaderboardNew() {
  ...
  const loadRankings = useCallback(async () => {
    ...
    try {
      ...
      if (rankingsData.length > 0) {
        // 🔑 מיון: ניקוד יורד, ובשוויון ניקוד — מיון אלפביתי (א'-ב'). סדר זה קובע את חלוקת הפרסים.
  ...
  const loadParticipantDetails = async (participantName) => {
    ...
    try {
      ...
      breakdown.forEach(item => {
        if (LOCATION_TABLE_IDS.includes(item.table_id)) {
          ...
          regularBreakdown.push({
            score:               item.score,
            max_score:           (q.table_id === 'T17') ? 14 : item.max_score,   // T17: מקס אמיתי 14 (D+E), לא possible_points (10)
            table_id:            item.table_id || '?',
      ...
      //   אם הנבחרת שניחשת היא השלישית בפועל → סופי רק כשגם ההעפלה (.1) ידועה
      //   (עד אז אפור — יכולה עוד לזכות +7). אחרת (רביעית/ראש/סגנית) → סופי.
      const predScoreFinal = (q, predText, allQ) => {
        const tbl = q.table_id;
        const qid = parseInt(q.question_id, 10);
        if ((tbl !== 'T16' && tbl !== 'T17') || !Number.isInteger(qid)) {
          return isScoreFinal(q, allQ);
        }
        const g = tbl === 'T16' ? Math.ceil(qid / 2) : qid;
        const resOf = (t, id) => {
          const x = allQ.find(z => z.table_id === t && z.question_id === id);
          return (x && x.actual_result && x.actual_result !== '__CLEAR__' && String(x.actual_result).trim() !== '')
            ? normT(x.actual_result) : null;
        };
        const head   = resOf('T16', String(2 * g - 1));
        const runner = resOf('T16', String(2 * g));
        const third  = resOf('T17', String(g));
        const advKnown = !!allQ.find(z => z.table_id === 'T17' && z.question_id === `${g}.1`
          && z.actual_result && z.actual_result !== '__CLEAR__' && String(z.actual_result).trim() !== '');
        if (!(head && runner && third)) return false;   // הבית טרם הוכרע → אפור
        const predC = normT(predText || '');
        if (predC && predC === third) return advKnown;  // ניחשת את השלישית → צריך גם העפלה
        return true;                                     // רביעית / כבר ראש/סגנית → סופי
      };
      ...
      const qualifyingSections = sortedTableIds.map((tableId, idx) => {
        ...
        // 🆕 קבוצות שנוספו לעולות מאז הקיבוע (לטבלאות-רשימה T19+): מי שניחש אותן — הרוויח עכשיו.
        //    ב-T16/T17 המשבצת מיקומית (בית ספציפי) ולכן שם התיוג נשאר לפי המשבצת עצמה.
        const newActualTeams = new Set(
          tSlots.filter(q => newSinceBaseline.has(q.id) && q.actual_result && q.actual_result !== '__CLEAR__')
                .map(q => normT(q.actual_result)));
        ...
        const preds = tSlots.map(q => {
          ...
          const isNewGain = useScoreService
            ? newSinceBaseline.has(q.id)
            : (isAdv && newActualTeams.has(norm));
          return { pred: disp, isAdv, isElim, pts: q.possible_points || 0, exactScore, exactFinal, useScoreService, isNew: isNewGain };
        });
      ...
      // 🆕 סיכום "מאז הקיבוע": שאלות רגילות + משבצות T16/T17 (ניקוד מדויק) + פגיעות חדשות ברשימות עולות (T19+, לפי pts). לא כולל בונוסים.
      let sinceBaseline = null;
      if (baselineSnap?.results) {
        const slotGain = p => p.useScoreService
          ? (typeof p.exactScore === 'number' && p.exactScore > 0 ? p.exactScore : 0)
          : (p.isAdv ? (p.pts || 0) : 0);
        const newRows  = regularBreakdown.filter(r => r.isNew);
        const newSlots = qualifyingSections.flatMap(sec => sec.preds.filter(p => p.isNew && slotGain(p) > 0));
        sinceBaseline = {
          sum:   newRows.reduce((s, r) => s + (r.score || 0), 0) + newSlots.reduce((s, p) => s + slotGain(p), 0),
  ...
  const getSortedRankings = () => [...rankings].sort((a, b) => {
    ...
    // 🔑 בשוויון — מיון אלפביתי (א'-ב'), עקבי עם חלוקת הפרסים
          ...
          <div style={{ padding: '14px 20px', overflowY: 'auto', flex: 1 }}>
            {loadingDetails ? (
              ...
              const newSlots = (participantDetails.qualifyingSections || []).flatMap(sec =>
                sec.preds.filter(p => p.isNew && slotGain(p) > 0)
                  .map(p => ({ desc: sec.tableDesc, pred: p.pred, score: slotGain(p) })));
              const items = [
